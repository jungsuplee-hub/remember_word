(function () {
  const GUIDE_CONTENT = {
    today: {
      title: '오늘의 학습 빠른 가이드',
      description:
        '당일 학습 계획과 바로가기 버튼을 한눈에 확인하고 학습을 시작할 수 있는 대시보드입니다.',
      sections: [
        {
          heading: '상단 정보 확인',
          items: [
            '오른쪽 상단에서 인사말과 로그아웃, 계정 관리 버튼을 확인할 수 있습니다.',
            '\"학습 계획 관리\" 버튼을 누르면 학습 계획 화면으로 이동해 일정을 조정합니다.',
            '초보자 가이드는 언제든 다시 눌러 도움말을 확인할 수 있습니다.'
          ]
        },
        {
          heading: '오늘의 계획 살펴보기',
          items: [
            '중앙 영역에서 오늘 날짜의 학습 계획을 확인하거나 비어 있으면 안내 메시지가 표시됩니다.',
            '계획이 없을 때는 \"학습 계획 설정\" 버튼으로 바로 일정을 추가할 수 있습니다.',
            '단어 암기 버튼을 눌러 바로 암기 화면으로 이동해 학습을 시작하세요.'
          ]
        },
        {
          heading: '메모와 시험 이력 활용',
          items: [
            '학습 계획에 메모를 추가해 두면 오른쪽 패널에 오늘의 메모가 표시됩니다.',
            '상단의 \"시험 이력\" 버튼이 활성화되면 최근 시험 결과를 모달에서 확인할 수 있습니다.',
            '다른 학습 도구는 상단 메뉴에서 빠르게 이동할 수 있습니다.'
          ]
        }
      ]
    },
    ...
    'study-plan': {
      title: '학습 계획 화면 둘러보기',
      description:
        '달력에서 날짜를 고르고 그룹을 지정해 하루 학습 일정을 준비할 수 있는 화면입니다.',
      sections: [
        {
          heading: '달력 탐색하기',
          items: [
            '상단 좌우 화살표로 월을 이동하고, 원하는 날짜를 클릭하면 계획 편집 창이 열립니다.',
            '이미 계획한 날짜는 달력에 강조되어 한눈에 확인할 수 있습니다.',
            '달력을 스크롤해도 헤더는 고정되어 현재 달을 쉽게 파악할 수 있습니다.'
          ]
        },
        {
          heading: '계획 편집',
          items: [
            '우측 팝업에서 폴더와 그룹을 선택해 하루 학습 목록을 추가합니다.',
            'Ctrl/⌘ 키를 누른 채 여러 그룹을 선택하면 한번에 여러 학습을 등록할 수 있습니다.',
            '더 이상 필요 없는 항목은 각 항목의 삭제 버튼으로 제거하거나 하루 계획 비우기를 이용하세요.'
          ]
        },
        {
          heading: '학습 메모 활용',
          items: [
            '하단 메모 영역에 학습 목표나 주의 사항을 적으면 오늘의 학습 화면에 함께 표시됩니다.',
            '메모 저장 후에는 자동으로 최신 상태가 반영되며, 메모 비우기로 쉽게 초기화할 수 있습니다.',
            '계획을 저장한 뒤 오늘의 학습 화면으로 이동하면 변경 내용을 바로 확인할 수 있습니다.'
          ]
        }
      ]
    },
